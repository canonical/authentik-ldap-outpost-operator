...
#### Scenario: Leader migrates legacy plaintext
- **WHEN** the leader finds legacy `outpost_token` peer data without a secret ID
- **THEN** it creates the application secret, verifies the token is readable by ID, writes the secret ID, and only then removes plaintext

#### Scenario: Migration verification fails
- **WHEN** the newly created secret cannot be read or does not contain the expected token
- **THEN** the leader retains legacy plaintext for a later retry
...
#### Scenario: Legacy peer password
- **WHEN** peer identity includes a legacy plaintext password
- **THEN** reconciliation uses it to publish the library secret and removes peer plaintext only after publication succeeds
...
- **THEN** the charm removes all current and legacy tracking keys
