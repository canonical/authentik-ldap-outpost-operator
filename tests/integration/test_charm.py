...
def test_deploy(charm: pathlib.Path, juju: jubilant.Juju):
    ...
    juju.deploy(charm.resolve(), app="authentik-ldap-outpost-operator", resources=resources)
    juju.wait(jubilant.all_active)


# If you implement authentik_ldap_outpost.get_version in the charm source,
# remove the @pytest.mark.skip line to enable this test.
# Alternatively, remove this test if you don't need it.
@pytest.mark.skip(reason="authentik_ldap_outpost.get_version is not implemented")
def test_workload_version_is_set(charm: pathlib.Path, juju: jubilant.Juju):
    """Check that the correct version of the workload is running."""
    version = juju.status().apps["authentik-ldap-outpost-operator"].version
